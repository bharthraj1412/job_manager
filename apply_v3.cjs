...
function robustReplace(name, search, replacement) {
  if (search.test(code)) {
...
// 1. Auto Gmail multi-scan + Login Summary
robustReplace(
  'Auto Gmail multi-scan on login + login summary state',
  /\/\/ Auto-send job digest ONCE when jobs first load after login\s*useEffect\(\(\) => \{[\s\S]*?\}\, \[\s*jobs\.length > 0 \? 1 : 0\s*\]\);/m,
...
robustReplace(
  ...
  /const\s+\[sq,\s*setSq\]\s*=\s*useState\([^)]*\);\s*const\s+\[sr,\s*setSr\]\s*=\s*useState\([^)]*\);/m,
...
robustReplace(
  ...
  /async\s+function\s+saveJob\(e\)\s*\{\s*e\.preventDefault\(\);/m,
...
// 6. Login Summary Banner
robustReplace(
  'Login summary banner JSX',
  /\{\/\*\s*Main content\s*\*\/\}\s*<main\s+style=\{\{/m,
...
// 7. Job row buttons
robustReplace(
  'Interview prep and follow-up buttons',
  /\{job\.status === "Interview" && <Btn\s+v="grn".*?>🎙 Prep<\/Btn>\}/m,
  `{job.status === "Interview" && <Btn v="grn" onClick={(e) => { e.stopPropagation(); generateInterviewPrep(job); }} sx={{ fontSize: 10, padding: '3px 8px' }}>🎙 Prep</Btn>}
                            {job.status === "Applied" && <Btn v="yel" onClick={(e) => { e.stopPropagation(); generateFollowUp(job); }} sx={{ fontSize: 10, padding: '3px 8px' }}>📧 Follow-up</Btn>}`
);

// 8. Salary benchmark (robust to new F/Inp structure)
// We look for: <F label="Salary"><Inp value={form.salary}
// and replace it to include the benchmark button
robustReplace(
  'Salary benchmark in Add/Edit modal',
  /<F\s+label="Salary">\s*<Inp\s+value=\{form\.salary\}\s+onChange=\{e\s*=>\s*upd\("salary",\s*e\.target\.value\)\}\s+placeholder="e\.g\. ₹6\s*LPA"\s*\/>\s*<\/F>/m,
...
// 9. Modals JSX (insert at end)
robustReplace(
  'Modals JSX',
  /export\s+default\s+function\s+Dashboard\s*\(\{\s*session\s*\}\)\s*\{[\s\S]*?(?=\s*<\/div>\s*\)\s*;\s*\})/m,
  (match) => match + `
...
// 10. Scan Gmail button
robustReplace(
  'Scan Gmail button in header',
  /\{tab === "search" && <Btn onClick=\{\(\) => setShowSearch\(true\)\}\s*sx=\{\{\s*background:\s*"rgba\(6,182,212,0\.12\)"[\s\S]*?>\+ Search Jobs<\/Btn>\}/m,
  `$&
            {tab === "jobs" && <Btn onClick={() => handleGmailMultiScan(false)} sx={{ background:'rgba(99,102,241,0.1)', border:'1px solid rgba(99,102,241,0.2)', color:'#a5b4fc', gap:6 }}>📧 Scan Gmail</Btn>}`
);


fs.writeFileSync('src/Dashboard.jsx', code, 'utf-8');
console.log('Done:', patchCount);
