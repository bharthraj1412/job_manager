// patch_gmail_scan_fix.cjs
const fs = require('fs');
const path = require('path');

const filePath = path.join(__dirname, 'src', 'Dashboard.jsx');
if (!fs.existsSync(filePath)) {
  console.error('ERROR: src/Dashboard.jsx not found');
  process.exit(1);
}
...
// ── FIX 1: Broken regex [[sS]*] → [\s\S]* ─────────────────────────
// The broken string uses backticks so we match it with a RegExp instead
var brokenPattern = /const match = text\.replace\(\/```json\|```\/g, ""\)\.trim\(\)\.match\(\/\[.sS.\*\]\/\)/g;
if (brokenPattern.test(code)) {
  code = code.replace(brokenPattern, 'const match = text.replace(/```json|```/g, "").trim().match(/\\[[\\s\\S]*\\]/)');
  console.log('OK: FIX 1 - broken regex replaced');
  fixes++;
} else {
  console.warn('SKIP: FIX 1 - pattern not found (may already be fixed)');
}
...
rep(
  ...
  '        { label: "Rejected",            q: \'(subject:"unfortunately" OR subject:"not moving forward" OR subject:"not selected" OR subject:"other candidates") (from:careers OR from:jobs OR from:hr OR from:noreply) -subject:newsletter -subject:unsubscribe\' },\n' +
  '        { label: "Applied",             q: \'(subject:"application received" OR subject:"thank you for applying" OR subject:"application submitted") -subject:newsletter -subject:unsubscribe -subject:"password reset" -subject:"verify your"\' },\n' +
  '        { label: "Screening",           q: \'(subject:"phone screen" OR subject:"screening call" OR subject:"initial call") (from:careers OR from:jobs OR from:recruiting OR from:hr OR from:talent) -subject:newsletter\' },\n' +
  '      ];'
);

// ── FIX 3: No silent job creation from unmatched emails ────────────
rep(
  'FIX 3 - No silent job creation',
  '        } else if (!matchedJob) {\n' +
...
// ── FIX 4: Accurate summary + silent-scan only notifies on changes ──
rep(
...
console.log('  FIX 1  Broken regex -> correct JSON array match');
console.log('  FIX 2  Gmail queries require hiring senders, exclude newsletters');
console.log('  FIX 3  Unmatched emails no longer create tracker entries');
console.log('  FIX 4  Accurate summary; auto-scan only notifies on real changes');
console.log('  FIX 5  Single-account scan excludes receipts/resets/promotions');
console.log('  FIX 6  Per-account queries tightened');
console.log('\nRun: npm run dev');
