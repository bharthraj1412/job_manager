...
Format: Professional letter. Opening hook, relevant experience paragraph, strong closing with CTA. No clichés.`,
  ...
  async function scanSingleAccount(account) {
    ...
    try {
      ...
      const results = await Promise.allSettled(
        QUERIES.map(({ label, q }) =>
          fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages?maxResults=15&q=${encodeURIComponent(q)}`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      const details = await Promise.allSettled(
        allMessages.slice(0, 20).map(msg =>
          fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${msg.id}?format=metadata&metadataHeaders=Subject&metadataHeaders=From&metadataHeaders=Date`, { headers: { Authorization: `Bearer ${token}` } })
